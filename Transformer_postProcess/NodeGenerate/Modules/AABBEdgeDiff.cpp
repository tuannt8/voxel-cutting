...
{
// 	// Hard code - Very bad performance
...
void AABBTreeEdgeDiff::findLeafNode(AABBNode* root, int edgeIdx, std::vector<AABBNode*>& node)
{
	if(root)
	{
		...
		if (func.isPointInBox(root->LeftDown, root->RightUp, edge[0])
			|| func.isPointInBox(root->LeftDown, root->RightUp, edge[1])
			|| func.isPointInBox(root->LeftDown, root->RightUp, (edge[0]+edge[1])/2.0))
		{
			if(root->End)
			{
				node.push_back(root);
...
AABBNode* AABBTreeEdgeDiff::findLeafNode( AABBNode* _note, int idx, bool debug )
{
	if (_note->End)
	{
		if (debug)
		{
			ASSERT(!(_note->IndexInLeafNode==idx));
			TRACE1("%d\n", _note->IndexInLeafNode);
		}
		if (_note->IndexInLeafNode == idx)
		{
			return _note;
		}
		else
	...
	{
		AABBNode * found = findLeafNode(_note->Left, idx, debug);
		if (!found)
		{
			findLeafNode(_note->Right, idx, debug);
		}
