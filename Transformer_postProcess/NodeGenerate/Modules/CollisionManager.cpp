...
bool CollisionManager::bUseHibridTest = true;
...
bool CollisionManager::collisionBtwSurfAndLineSeg_part_T( SurfaceObj* obj, Vec3f l1, Vec3f l2, double radius, double margin )
{
	GeometricFunc func;
	AABBNode* root=obj->getBVH()->root();
	std::vector<Vec3f>* point=obj->point();
...
// 	// Tuan-center line model: We disable the area around hole point
// 	{
// 		arrayInt faceIdxs = obj->m_invisibleTriIdxs;
// 		VectorFunc vecFunc;
// 		for (int i=0; i<faceIdxs.size(); i++)
// 		{
// 			int idx = vecFunc.indexOfElement(&pCollisionTriIdx, faceIdxs[i]);
// 			if (idx!= -1)
// 			{
// 				pCollisionTriIdx.erase(pCollisionTriIdx.begin()+idx);
// 			}
// 		}
// 	}
// 	//
...
bool CollisionManager::collisionBtwSurfAndLineSeg_part( SurfaceObj* obj, Vec3f l1, Vec3f l2, double radius, double margin, bool bHidden )
{
	GeometricFunc func;
	AABBNode* root=obj->getBVH()->root();
	std::vector<Vec3f>* point=obj->point();
...
// 	// Tuan- hybrid collision manager
// 	if (bHidden)
// 	{
// 		arrayInt faceIdxs = obj->m_invisibleTriIdxs;
// 		VectorFunc vecFunc;
// 		std::vector<int>::iterator  it = pCollisionTriIdx.begin();
// 		while(it != pCollisionTriIdx.end())
// 		{
// 			int hIdx = vecFunc.indexOfElement(&faceIdxs, *it);
// 			if (hIdx==-1)
// 			{
// 				it = pCollisionTriIdx.erase(it);
// 			}
// 			else
// 				it++;
// 		}
...
void CollisionManager::printBVHIdx( AABBNode* root )
{
	if (!root)
		return;
	...
	if(root->End)
	{
		TRACE1("%d\n", root->IndexInLeafNode);
	}

	printBVHIdx(root->Left);
	printBVHIdx(root->Right);
}

bool CollisionManager::isSurfaceWithAABBIntersectBox(AABBNode* node, Box b, std::vector<Vec3f> *points, std::vector<Vec3i> *face)
{
	GeometricFunc geoF;
	Box nodeBox(node->LeftDown, node->RightUp);

	if (geoF.isBoxInBox(b.leftDown, b.rightUp, node->LeftDown, node->RightUp))
	{
		// Box b is large and cover the entire node, then it is intersect
		return true;
	}
	
	if(geoF.collisionBtwBox(nodeBox, b))
	{
		if (node->End) // Leaf node
		{
			// Test if the triangle really intersect
			return (geoF.isTriCollideBox(points, face->at(node->IndexInLeafNode), b));
		}
		else
		{
			// Check its children
			if (isSurfaceWithAABBIntersectBox(node->Left, b, points, face))
			{
				return true;
			}
			if (isSurfaceWithAABBIntersectBox(node->Right, b, points, face))
			{
				return true;
			}
		}
