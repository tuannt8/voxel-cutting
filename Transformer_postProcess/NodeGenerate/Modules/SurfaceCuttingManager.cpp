...
void SurfaceCuttingManager::cutting_1(SurfaceObj* obj, std::vector<Vec3f>* cutPoint, std::vector<Vec3i>* cutFace, std::vector<Vec3f>* cutFaceNormal, std::vector<int>* pointsOnCutFront)
{
	VectorFunc func;
	GeometricFunc geoFunc;
	CollisionManager collision;
	...
	// 1. Surface object data
	std::vector<Vec3f> *objPoint = obj->point();
...
void SurfaceCuttingManager::intersectionBtwSurfEdgeAndCutSurf(std::vector<int>& colTriIdxInSurfObj, std::vector<int>& colTriIdxInCutSurf, 
							std::vector<CutFace>& cutFaceInSurfObj, std::vector<CutFace>& cutFaceInCutFace,std::vector<int>& colEdgeIdx)
{
	TopologyContainer* container=Obj->container();
	std::vector<Vec3f>* surfNode=Obj->point();
		...
		if(cutFaceInSurfObj[i].faceIdx>0)
		{
			...
			else if(cutFaceInSurfObj[i].pointsOnEdgeNormalDirec.size()==1)
			{
				...
				for(int j=0;j<cutFaceInSurfObj[i].pointsOnFaceNormalDirec.size();j++)
				{
					...
					p=AddedPoint[idx];
					int test = pointsOnCutFront->size() - 1;
