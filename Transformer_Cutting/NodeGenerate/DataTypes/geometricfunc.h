...
public:
	...
	bool GeometricFunc::isTriSame(Vec3i tri1, Vec3i tri2)
	{
		int count=0;
		for (int i=0; i<3; i++)
		{
			if (tri1[i]==tri2[0] ||tri1[i]==tri2[1] ||tri1[i]==tri2[2])
			{
				count++;
			}
		}
	...
	static bool GeometricFunc::isPointInTri(Vec3f point, Vec3f tri[])
	{
	...
	bool GeometricFunc::isBoxInBox(Vec3f& leftDownBig, Vec3f& rightUpBig, Vec3f _leftDown, Vec3f _RightUp)
	{
		if(isPointInBox(leftDownBig, rightUpBig, _leftDown)&&isPointInBox(leftDownBig, rightUpBig, _RightUp))
			return true;
		else
			return false;
	}
	...
	bool GeometricFunc::isPointInSurf(std::vector<Vec3f>* point, std::vector<Vec3i>* face, AABBNode* root, Vec3f pos)
	{
		...
		int count=0;
		for(unsigned int i=0;i<pCollisionTriIdx.size();i++)
		{
			Tri tri;
	...
	static Vec3f GeometricFunc::computeNormal(Vec3f p[])
	{
		Vec3f v1=p[1]-p[0];
		Vec3f v2=p[2]-p[0];
	...
	static bool GeometricFunc::collisionBtwLinesegAndTri(Vec3f l1, Vec3f l2, Vec3f tri[], Vec3f& intersection)
	{
		Vec3f direc=l2-l1;
		Vec3f normal=computeNormal(tri);
	...
	bool GeometricFunc::collisionBtwTriAndLine(Vec3f pos, Vec3f direc, Tri& tri, Vec3f& intersection)
	{
	...
	bool GeometricFunc::collisionBtwTriDirectionalLine(Vec3f pos, Vec3f direc, Tri& tri, Vec3f& intersection)
	{
		...
		if(t>0)
		{
			...
			if (isPointOnTriEdge(intersection, tri.p))
			{
				return true;
			}
		}
	...
	bool isPointOnTriEdge(Vec3f& point, Vec3f tri[])
	{
		//point가 사각형 내부에 있는지 검사하는 함수
		Vec3f cross[3];
		cross[0] = (tri[1] - tri[0]).cross(point - tri[0]);
		...
		return cross[0].norm2() < ZERO_F
			|| cross[0].norm2() < ZERO_F
			|| cross[0].norm2() < ZERO_F;
	}
	...
	static float distanceBtwLineAndLineSeg(Vec3f l1, Vec3f l2, Vec3f p1, Vec3f p2)
	{
		...
		if (abs(detA) < EPS)//parallel?
		{
			...
			float ss1 = std::min(s1,s2), ss2 = std::max(s1,s2);
			if (ss1 > 1){
				pt1 = l2;
				pt2 = p1;
			}
		...
		Vec3f* pOnTri = NULL, Vec3f* pOnLine = NULL ) 
	{
		...
		{
			Vec3f triPoint[3];
			triPoint[0]=(*points)[tri[0]];
			triPoint[1]=(*points)[tri[1]];
			triPoint[2]=(*points)[tri[2]];
			Vec3f intersect;
			if (collisionBtwLinesegAndTri(l1,l2,triPoint, intersect))
			{
				if (pOnTri)
					(*pOnTri) = intersect;
	...
	double projectOnPlane_withSign(Vec3f Normal,Vec3f Center,Vec3f Point)
	{
	...
	bool isSamePoint(Vec3f pt1, Vec3f pt2)
	{
		Vec3f pt = pt2 - pt1;
		return (pt[0]*pt[0] + pt[1]*pt[1] + pt[2]*pt[2]) < 0.00001;
	}

	bool isVec1LargerThanVec2(Vec3f pt1, Vec3f pt2)
	{
		return pt1[0] > pt2[0] || pt1[1] > pt2[1] || pt1[2] > pt2[2];
	}

	void sortStraighPointByX(arrayVec3f &intersectedPts)
	{
		#define INVALID_INDEX -1
		// Sequence search. Should apply binary search in sorted array
		arrayVec3f sortedPts;

		for (int i = 0; i < intersectedPts.size(); i++)
		{
			Vec3f pt = intersectedPts[i];
			int idx = 0;
			for (idx = 0; idx < sortedPts.size(); idx++)
			{
				if (isSamePoint(pt, sortedPts[idx]))
				{
					idx = INVALID_INDEX;
					break;
				}
				if ((pt[0] > sortedPts[idx][0]))
				{
					idx++;
					break;
				}
			...
			{
				sortedPts.insert(sortedPts.begin()+idx, pt);
			}
		}

		intersectedPts = sortedPts;
	}

	bool intersectionBtwLineAndLineSegment(Vec3f lpt, Vec3f ldirect, Vec3f l1, Vec3f l2, Vec3f &intersection)
	{
		

		// Does they intersect first, they must be in same plan
		Vec3f l12=l2-l1;
		Vec3f normV=l12.cross(ldirect);

		if ((lpt - l1)*normV > ZERO_F) // Not in same plan
			return false;

		// Find intersection
		float s=((lpt-l1)*normV)/(l12*normV);
		intersection=l1+l12*s;
		return s > 0 && s < 1;
	}

	void fitBoxToCoverBox(Box & boxBig, Vec3f leftDownPoint, Vec3f rightUpPoint)
	{
		for (int j = 0; j < 3; j++)
		{
			if (boxBig.leftDown[j] > leftDownPoint[j])
			{
				boxBig.leftDown[j] = leftDownPoint[j];
			}
			if (boxBig.rightUp[j] < rightUpPoint[j])
			{
				boxBig.rightUp[j] = rightUpPoint[j];
			}
		}
	}

	bool isTriCollideBox(std::vector<Vec3f> * points, Vec3i tri, Box b)
	{
		// Transform box to unit box
		Vec3f trans = b.center;
		Vec3f scale = b.rightUp - b.leftDown;

		// tri
		tri_box_test::Triangle3 tri3;
		tri_box_test::Point3 pt[3];
		for (int i = 0; i < 3; i++)
		{
			pt[i].x = (points->at(tri[i])[0] - trans[0])/scale[0];
			pt[i].y = (points->at(tri[i])[1] - trans[1])/scale[1];
			pt[i].z = (points->at(tri[i])[2] - trans[2])/scale[2];
		}
		...
		return tri_box_test::t_c_intersection(tri3) == 0;
	}

	void combineBoxi(Vec3i &destLDi, Vec3i &destRUi, Vec3i sourceLDi, Vec3i sourceRUi)
	{
		for (int i = 0; i < 3; i ++)
		{
			if (destLDi[i] > sourceLDi[i])
			{
				destLDi[i] = sourceLDi[i];
			}
			if (destRUi[i] < sourceRUi[i])
			{
				destRUi[i] = sourceRUi[i];
			}
		}
	}
