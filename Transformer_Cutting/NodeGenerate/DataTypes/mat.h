...
class Mat_F : public FixedArray<VecNoInit<C,real>,R>
{
public:
	enum { N = R*C };
	...
	Mat_F(Row r1, Row r2, Row r3, Row r4)
	{
		this->elems[0]=r1;
		this->elems[1]=r2;
		this->elems[2]=r3;
	...
	template<typename real2> void operator=(const Mat_F<R,C,real2>& m)
	{
		std::copy(m.begin(), m.begin()+R, this->begin());
	}

	/// Assignment from a matrix of different size.
	template<int R2, int C2> void operator=(const Mat_F<R2,C2,real>& m)
	{
		std::copy(m.begin(), m.begin()+(R>R2?R2:R), this->begin());
	}

	template<int R2, int C2> void getsub(int R0, int C0, Mat_F<R2,C2,real>& m) const
	{
		for (int i=0;i<R2;i++)
			for (int j=0;j<C2;j++)
	...
	template<int R2, int C2> void setsub(int R0, int C0, const Mat_F<R2,C2,real>& m)
	{
		for (int i=0;i<R2;i++)
			for (int j=0;j<C2;j++)
	...
	void transpose(const Mat_F<C,R,real> &m)
	{
		for (int i=0;i<R;i++)
			for (int j=0;j<C;j++)
	...
	Mat_F<C,R,real> transposed() const
	{
		Mat_F<C,R,real> m(NOINIT);
		for (int i=0;i<R;i++)
			for (int j=0;j<C;j++)
	...
	bool operator==(const Mat_F<R,C,real>& b) const
	{
		for (int i=0;i<R;i++)
	...
	template <int P>
	Mat_F<R,P,real> operator*(const Mat_F<C,P,real>& m) const
	{
		Mat_F<R,P,real> r(NOINIT);
		for(int i=0;i<R;i++)
			for(int j=0;j<P;j++)
			{
	...
	Mat_F<R,C,real> operator+(const Mat_F<R,C,real>& m) const
	{
		Mat_F<R,C,real> r(NOINIT);
		for(int i = 0; i < R; i++)
			r[i] = (*this)[i] + m[i];
		return r;
	}
	...
	template <int P>
	Mat_F<C,P,real> multTranspose(const Mat_F<R,P,real>& m) const
	{
		Mat_F<C,P,real> r(NOINIT);
		for(int i=0;i<C;i++)
			for(int j=0;j<P;j++)
			{
	...
	Mat_F<R,C,real> operator*(real f) const
	{
		Mat_F<R,C,real> r(NOINIT);
		for(int i=0;i<R;i++)
			for(int j=0;j<C;j++)
	...
	friend Mat_F<R,C,real> operator*(real r, const Mat_F<R,C,real>& m)
	{
		return m*r;
	}
	...
	void operator +=(const Mat_F<R,C,real>& m)
	{
		for(int i=0;i<R;i++)
			this->elems[i]+=m[i];
	}
	...
	void addTransposed(const Mat_F<R,C,real>& m)
	{
		for(int i=0;i<R;i++)
			for(int j=0;j<R;j++)
				(*this)[i][j] += m[j][i];
	...
	void subTransposed(const Mat_F<R,C,real>& m)
	{
		for(int i=0;i<R;i++)
			for(int j=0;j<R;j++)
				(*this)[i][j] += m[j][i];
	...
	/// Invert matrix m
	bool invert(const Mat_F<R,C,real>& m)
	{
		return invertMatrix(*this, m);
	}

	static Mat_F<R,C,real> transformTranslation(const Vec<C-1,real>& t)
	{
		Mat_F<R,C,real> m;
		m.identity();
		for (int i=0;i<C-1;++i)
	...
	static Mat_F<R,C,real> transformScale(real s)
	{
		Mat_F<R,C,real> m;
		m.identity();
		for (int i=0;i<C-1;++i)
	...
	static Mat_F<R,C,real> transformScale(const Vec<C-1,real>& s)
	{
		Mat_F<R,C,real> m;
		m.identity();
		for (int i=0;i<C-1;++i)
...
class MatNoInit : public Mat_F<R,C,real>
{
public:
	MatNoInit()
		: Mat_F<R,C,real>(NOINIT)
	{
	}
	...
	/// Assignment from another matrix
	template<int R2, int C2, typename real2> void operator=(const Mat_F<R2,C2,real2>& m)
	{
...
template<class real>
inline real determinant(const Mat_F<3,3,real>& m)
{
...
template<class real>
inline real determinant(const Mat_F<2,2,real>& m)
{
	return m(0,0)*m(1,1)
		- m(1,0)*m(0,1);
}
...
bool invertMatrix(Mat_F<S,S,real>& dest, const Mat_F<S,S,real>& from)
{
	int i, j, k;
	Vec<S,int> r, c, row, col;

	Mat_F<S,S,real> m1 = from;
	Mat_F<S,S,real> m2;
	m2.identity();
...
template<class real>
bool invertMatrix(Mat_F<3,3,real>& dest, const Mat_F<3,3,real>& from)
{
...
template<class real>
bool invertMatrix(Mat_F<2,2,real>& dest, const Mat_F<2,2,real>& from)
{
	real det=determinant(from);
...
typedef Mat_F<1,1,float> Mat1x1f;
typedef Mat_F<1,1,double> Mat1x1d;

typedef Mat_F<2,2,float> Mat2x2f;
typedef Mat_F<2,2,double> Mat2x2d;

typedef Mat_F<3,3,float> Mat3x3f;
typedef Mat_F<3,3,double> Mat3x3d;
typedef Mat_F<3,3,int> Mat3x3i;

typedef Mat_F<3,4,float> Mat3x4f;
typedef Mat_F<3,4,double> Mat3x4d;

typedef Mat_F<4,4,float> Mat4x4f;
typedef Mat_F<4,4,double> Mat4x4d;
...
void svddcmp(Mat_F<m,n,Real> &a, Vec<n,Real> &w, Mat_F<n,m,Real> &v)
{
	bool flag;
