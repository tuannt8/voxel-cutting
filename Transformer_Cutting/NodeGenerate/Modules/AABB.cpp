...
void AABBTree::updateDepth(AABBNode* node)
{
	if(node)
	{
		if(node->Parent)
...
void AABBTree::printLeafNodeIdx(int mode)
{
	printLeafNodeRescursive(Root, mode);
}

void AABBTree::printLeafNodeRescursive( AABBNode* node, int mode )
{
	if (node)
	{
		if (node->End)
		{
			if (mode==0)
			{
				TRACE1("%d\n", node->IndexInLeafNode);
			}
			else if (mode == 1)
			{
				CStringA a;
				a.Format("%d \t %lf %lf %lf - %lf %lf %lf", node->IndexInLeafNode,
					node->LeftDown[0], node->LeftDown[1], node->LeftDown[2],
					node->RightUp[0], node->RightUp[1], node->RightUp[2]);
				TRACE1("%s\n", a.GetBuffer());
			}
		}
		else
		{
			printLeafNodeRescursive(node->Left, mode);
			printLeafNodeRescursive(node->Right, mode);
		}
	}
}
